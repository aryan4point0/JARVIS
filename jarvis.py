...
def open_spotify_search(query):
    # Note: This Spotify URL appears malformed/incorrect in the original code,
    # but I'm keeping it as you provided it. It should ideally be a valid Spotify search URL.
...
def processCommand(c):
    ...
    # --- Sports Scores and News (NEW ADDITION) ---
    ...
    if "football score" in cl or "football scores" in cl or "soccer scores" in cl:
...
def create_hud():
    ...
    title = tk.Label(header, text="🎛️ JARVIS HUD", bg=BG, fg=FG, font=("Segoe UI", 13, "bold"))
    ...
    def close_hud():
        if messagebox.askyesno("Exit Jarvis", "Close Jarvis HUD and stop voice listening?"):
            root.destroy()
            os._exit(0)
    ...
    def reload_and_update():
        ok, _ = load_music()
        if ok:
            messagebox.showinfo("Jarvis", "✅ Music library reloaded successfully!")
            fill_song_list()
        else:
            messagebox.showerror("Jarvis", "❌ Failed to reload music library.")
    ...
    def play_from_input():
        ...
        if not q:
            messagebox.showwarning("Jarvis", "Please enter a song name.")
            return
    ...
    hint = tk.Label(root, text="Say: 'Jarvis play <song>' or 'Jarvis open <website>' or 'Jarvis cricket scores'",
